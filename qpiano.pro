...
CONFIG(debug, debug|release): DEFINES += DEBUG_LOG_TRACE DEBUG_LOG_TIMING
...
isEmpty(DATA_DIR){
    DATA_DIR=share/games/qpiano
}
message("DATA_DIR:" $$DATA_DIR)
DEFINES += DATA_DIR="\\\""$$DATA_DIR"\\\""

RC_FILE = src/qpiano.rc
...
unix {
   ...
   contains(NO_DOCS, OFF){
      ...
      docs.path = $$PREFIX/share/doc/qpiano
   ...
   contains(NO_LICENSE, OFF){
      ...
      license.path = $$PREFIX/share/licenses/qpiano
   ...
   contains(NO_CHANGELOG, OFF){
      ...
      changelog.path = $$PREFIX/share/doc/qpiano
   ...
   contains(WITH_TIMIDITY, ON){
      ...
      timidity.files = tools/timidity/qpiano-timidity
      INSTALLS += timidity

      timidity_desktop.path = $$PREFIX/share/applications
      timidity_desktop.files = tools/timidity/qpiano-timidity.desktop
   ...
   contains(WITH_FLUIDSYNTH, ON){
      ...
      fluidsynth.files = tools/fluidsynth/qpiano-fluidsynth
      INSTALLS += fluidsynth

      fluidsynth_desktop.path = $$PREFIX/share/applications
      fluidsynth_desktop.files = tools/fluidsynth/qpiano-fluidsynth.desktop
   ...
   contains(NO_LANGS, OFF){
	   ...
	   data_langs_fix.extra = rm ${INSTALL_ROOT}$$PREFIX/$$DATA_DIR/translations/music_blank.qm \
               ${INSTALL_ROOT}$$PREFIX/$$DATA_DIR/translations/qpiano_blank.qm
	   INSTALLS += data_langs_fix
   }

   desktop.path = $$PREFIX/share/applications
   desktop.files = qpiano.desktop

   icon32.path = $$PREFIX/share/icons/hicolor/32x32/apps
   icon32.files = icons/hicolor/32x32/qpiano.png

   icon48.path = $$PREFIX/share/icons/hicolor/48x48/apps
   icon48.files = icons/hicolor/48x48/qpiano.png
